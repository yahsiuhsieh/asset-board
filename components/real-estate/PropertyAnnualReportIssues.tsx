...
function IssueRow({ issue }: { issue: AnnualQualityIssue }) {
  ...
  return (
    <div className="grid gap-1 border-b border-border/70 px-4 py-3 last:border-b-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
      ...
      <div className="sm:justify-self-end">
        {display.meta ? (
          <span className="inline-flex rounded-md bg-secondary px-2 py-1 text-xs font-semibold text-muted-foreground">
            {display.meta}
          </span>
        ) : null}
      </div>
...
function IssueGroup({
  ...
  const headingClassName =
    tone === "blocking"
      ? "bg-red-50/80 text-red-700 dark:bg-red-950/35 dark:text-red-300"
      : "bg-amber-50/80 text-amber-700 dark:bg-amber-950/35 dark:text-amber-300";

  return (
    <div>
      <div
        className={`flex items-center justify-between gap-3 border-b border-border/70 px-4 py-2 ${headingClassName}`}
      >
    ...
    <Card className="border-border bg-card">
      <CardHeader className="flex flex-col gap-3 space-y-0 sm:flex-row sm:items-center sm:justify-between">
        <button
          ...
          type="button"
        >
          <span className="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-amber-100 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/35 text-amber-700 dark:text-amber-300">
          ...
          <span className="min-w-0">
            <span className="flex flex-wrap items-center gap-2">
              ...
              {blockingIssueCount > 0 ? (
                <span className="rounded-md border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700 dark:border-red-800/70 dark:bg-red-950/35 dark:text-red-300">
                  {blockingIssueCount} blocking
                </span>
              ) : null}
              {warningIssueCount > 0 ? (
                <span className="rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700 dark:border-amber-800/70 dark:bg-amber-950/35 dark:text-amber-300">
